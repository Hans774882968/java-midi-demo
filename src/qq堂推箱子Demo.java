...
public class qq堂推箱子Demo {
    public static void main(String[] args)
        ...
        qq堂推箱子right(track, channel, myNoteImpl, volume, beat, length);
    ...
    public static void qq堂推箱子right(
        ...
        tick += addNote(track, channel, myNoteImpl.getMi(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getFa(1), volume, tick, beat / 4, tick + length);
        ...
        tick += addNote(track, channel, myNoteImpl.getSi(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getDo(2), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSi(1), volume, tick, beat / 2, tick + length);
        ...
        // 3
        tick += addNote(track, channel, 0, 0, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLa(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSi(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getDo(2), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLa(1), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLa(1), volume, tick, beat / 2, tick + length);
        ...
        tick += addNote(track, channel, myNoteImpl.getSol(1), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLa(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(1), volume, tick, beat / 2, tick + length);
        ...
        tick += addNote(track, channel, myNoteImpl.getRe(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getDo(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSi(0), volume, tick, beat, tick + length);

        tick += addNote(track, channel, myNoteImpl.getSi(0), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSi(0), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLa(0), volume, tick, beat / 4, tick + length);
        ...
        tick += addNote(track, channel, 0, 0, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(0), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLa(0), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSi(0), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(0), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLa(0), volume, tick, beat / 4, tick + length);
        ...
        // 11
        tick += addNote(track, channel, 0, 0, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getFa(0), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(0), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLa(0), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getFa(0), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(0), volume, tick, beat / 4, tick + length);
        ...
        tick += addNote(track, channel, myNoteImpl.getDo(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getRe(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getReUp(1), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getDo(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getRe(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getReUp(1), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getDo(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getRe(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getReUp(1), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSolUp(1), volume, tick, beat / 2, tick + length);

        tick += addNote(track, channel, 0, 0, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getReUp(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getFa(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(1), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getReUp(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getFa(1), volume, tick, beat / 4, tick + length);
        ...
        tick += addNote(track, channel, myNoteImpl.getFa(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(1), volume, tick, beat / 2, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLaUp(1), volume, tick, beat / 2, tick + length);
        ...
        tick += addNote(track, channel, myNoteImpl.getFa(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSolUp(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSol(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getFa(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getReUp(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getRe(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getDo(1), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getLaUp(0), volume, tick, beat / 4, tick + length);
        tick += addNote(track, channel, myNoteImpl.getSolUp(0), volume, tick, beat / 4, tick + length);
